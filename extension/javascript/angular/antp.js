      ...
      angular.forEach(tiles, function(tile, id) {
        ...
        /* Start :: CSS */
          ...
          if ( tile.type === "app" || tile.type === "shortcut" ) {
            if ( tile.shortcut_background_transparent === true ) {
              ...
              tile.css.bg = "background-image: url("+tile.img+"), -webkit-gradient(linear, 100% 100%, 0% 0%, to(rgba(255, 255, 255, 0.05)), from(rgba(255, 255, 255, 0.15))); background-color: "+tile.color+";";
            }
          }
        ...
        switch ( tile.type ) {
          case "iframe":
            ...
            if (firstLoad) {
              const eventWidgetIdString = `${tile.name} ${ (tile.parent_id || tile.instance_id || tile.id) }`;
...
/* START :: Recently Closed Tabs Menu */
  ...
  ajs.controller("RCTMCtrl", ["$scope", function($scope) {
    ...
    $scope.removeTab = function(tabToRemove) {
      const index = this.recently_closed.indexOf(tabToRemove);
...
/* START :: Backup directive */
  ajs.controller("backupsCtrl", ["$scope", function($scope) {
        ...
        setTimeout(function() {
          ...
          chrome.storage.sync.getBytesInUse(null, function(bytes) {
            if (!checkForErrors(chrome.runtime.lastError)) {
              const spaceStr = (bytes / 1024).toFixed(2) + "KB of " + (chrome.storage.sync.QUOTA_BYTES / 1024).toFixed(2) + "KB used";
    ...
    function checkForErrors(error) {
      if (error) {
        if (error.message.includes("MAX_SUSTAINED_WRITE_OPERATIONS_PER_MINUTE") || error.message.includes("MAX_WRITE_OPERATIONS_PER_HOUR")) {
        ...
        } else if (error.message.includes("QUOTA_BYTES") || error.message.includes("MAX_ITEMS")) {
      ...
      validateBackupName(backupName, function(valid) {
        if (valid) {
          buildExportString(function(exportString) {
            const chunks = makeChunks(backupName, exportString, chrome.storage.sync.QUOTA_BYTES_PER_ITEM);
            const backupKey = "backup_" + backupName;
            chunks[backupKey] = {
              "name": backupName,
            ...
            chrome.storage.sync.get(name, function(backup) {
              if (!checkForErrors(chrome.runtime.lastError)) {
                ...
                chrome.storage.sync.get(keys, function(chunks) {
                  if (!checkForErrors(chrome.runtime.lastError)) {
                    const importString = Object.keys(chunks).map(function (key) { return chunks[key]; }).join('');
