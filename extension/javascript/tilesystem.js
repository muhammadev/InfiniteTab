...
function placeGrid(storage_data) {
  ...
  if ( typeof(window.innerHeight) !== "undefined"
    ...
    const res_height = Math.floor((window.innerHeight - GRID_MARGIN_TOP) / (GRID_TILE_SIZE + (GRID_TILE_PADDING * 2)));
    const res_width = Math.floor((window.innerWidth - GRID_MARGIN_LEFT) / (GRID_TILE_SIZE + (GRID_TILE_PADDING * 2))) + 3;
  ...
  if ( typeof(screen.width) !== "undefined"
    ...
    const res_height2 = Math.floor((screen.height - 180 - GRID_MARGIN_TOP) / (GRID_TILE_SIZE + (GRID_TILE_PADDING * 2)));
    const res_width2 = Math.floor((screen.width - GRID_MARGIN_LEFT) / (GRID_TILE_SIZE + (GRID_TILE_PADDING * 2))) + 3;

    height = Math.max(res_height2, height);
    width = Math.max(res_width2, width);
  }
  ...
  if ( typeof tiles === "object" ) {
    $.each(tiles, function(id, widget) {
      placed_height = Math.max(placed_height, parseFloat(widget.where[0]) + parseFloat(widget.size[0]))
      placed_width = Math.max(placed_width, parseFloat(widget.where[1]) + parseFloat(widget.size[1]) + 3);
    });
  ...
  if ( parseInt(storage_data.settings.grid_width) % 1 === 0 )
    width = Math.max(parseInt(storage_data.settings.grid_width), 4);

  if ( parseInt(storage_data.settings.grid_height) % 1 === 0 )
    height  = Math.max(parseInt(storage_data.settings.grid_height), 3);
...
function findClosest(tile){
  let closestElm = null;
  let thisTile = $(tile).position();
  const boxX = thisTile.left,
        boxY = thisTile.top;
  let distElm = -1;
  $(".tile").each(function(ind, elem){
    let thisElem = $(elem).position();
...
function getCovered(tile) {
  ...
  const top = parseInt($(closestElm).attr("land-top"), 10);
  const left = parseInt($(closestElm).attr("land-left"), 10);

  const height = parseInt($(tile).attr("tile-height"), 10);
  const width = parseInt($(tile).attr("tile-width"), 10);
  ...
  $(document).on("mousemove", function(e) {
    ...
    if (typeof (held_element.element) === "object") {
      if (update === true) {
        ...
        let held_left = held_element.width / 2;
        let held_top = held_element.height / 2;
...
/* START :: Lock */
  let lock = true;
  $(document).ready(function() {
    localStorage.setItem("locked", "true") // No more persistent lock / unlock
